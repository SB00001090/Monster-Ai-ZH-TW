...
/**
 * Invoke LLM with user's configuration or fallback to Manus LLM
 */
export async function invokeLLMWithConfig(
  messages: Message[],
  userConfig: LLMConfig | null | undefined
): Promise<LLMResponse> {
  // If no user config or invalid, use Manus LLM
  if (!isLLMConfigValid(userConfig)) {
    return invokeManusLLM({ messages });
  }

  try {
    return await invokeProviderLLM(messages, userConfig as LLMConfig);
  } catch (error) {
    console.error("Error invoking user LLM, falling back to Manus LLM:", error);
